...
public class StaticList<E> implements List<E> {
	...
	public int contaElementos(E el) throws IllegalArgumentException {
		if(elements == null) throw new IllegalArgumentException("O array 'elements' não pode ser nulo!");
		return contaElementos(el, 0, 0); 
	}

	private int contaElementos(E el, int i, int j) {
		if(i >= numElements)
			return j;
		if(elements[i] != null && elements[i].equals(el)) {
			j++;
		}
		return contaElementos(el, i + 1, j);
	}
